...
def plot_score_comparisons(experiment_path, consistencies, scores_certain, scores_full, std_certain, std_full, max_possible_score, existing_score,
    ...
    fig, ax = plt.subplots()
    title = "accuracy"
    l1 = ax.plot(consistencies, max_possible_scores, color='black', label='Upper Bound')
    l2 = ax.plot(consistencies, existing_scores, color='green', label='Plug-in-model')
    l3 = ax.plot(consistencies, scores_certain, color='blue', label='Consistent dataset-model')
    ...
    l4 = ax.plot(consistencies, scores_full, color='lightgreen', label='Relabeled dataset-model')
