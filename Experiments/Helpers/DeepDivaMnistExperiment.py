...
class DeepDivaMnistExperiment:
    ...
    def main(self):
        adaptor = DeepDIVADatasetAdaptor(config.MNIST_PATH_ORIGINAL)
        test_adaptor = DeepDIVADatasetAdaptor(config.MNIST_PATH_TEST)
        # run in deepdiva before using
        # python get_a_dataset.py --dataset mnist --output-folder /dd_resources/data/

        X_y = adaptor.read_folder_dataset()
        X_y_test = test_adaptor.read_folder_dataset()

        train_test_split(X_y)
        X_y_existing_model, X_y_unseen = train_test_split(X_y, test_size=0.90, random_state=42)
        X_y_to_annotate, X_y_test = train_test_split(X_y_unseen, test_size=0.40, random_state=42)

        self.train_exising_model(X_y_existing_model)
        self.train_ground_truth_model(X_y_to_annotate)
        self.make_annotations(X_y_to_annotate)
        ...
        for certainty in certainties:
            train, val, test = self.train_certain_split
            train, val, test = self.train_full_split
